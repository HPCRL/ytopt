...
print("Mean: ", 1/x.mean())
print("The best configurations (for the smallest 1/accuracy) of P1, P2, P3 and P4 is:\n")
print("BF\t\tBN\t\tBOW\t\tTC\t\tTF\t\tTKW\t\tTN\t\tTOW\t\texecution time\t\tscore\n")
mn = x.min()
for i in range(len(new_array)): 
   if x[i] == mn:
      ...
      print(f"#define BOW {int(new_array[i,2])}")

      print(f"#define TN {int(new_array[i,6])}")
      print(f"#define TF {int(new_array[i,4])}")
      print(f"#define TOW {int(new_array[i,7])}")

      print(f"#define TC {int(new_array[i,3])}")
      print(f"#define TKW {int(new_array[i,5])}")
