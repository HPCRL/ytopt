...
class Plopper:
    def __init__(self, sourcefile, outputdir, bookkeeping):
        ...
        self.autotuner_lowlevel_td_spec_file = bookkeeping[
            "autotuner_lowlevel_td_spec_file"
        ]
    ...
    def findRuntime(self, x, params):
        ...
        lowlevel_transform_ir_file = (
            sourcefile_dir + "/build/" + self.autotuner_lowlevel_td_spec_file
        )
        ...
        if return_value == 0:
            ...
            if compile_return_value == 0:
                execution_status = subprocess.run(
                    run_cmd, shell=True, stdout=subprocess.PIPE, encoding="utf-8"
                )
                ...
                print(
                    "Compile Command failed with exit status:",
                    os.WEXITSTATUS(compile_return_value),
                )
                with open(ireelogfile, "a") as myfile:
                    myfile.write("###########################")
                    myfile.write(compile_cmd)
            ...
            print(
                "Build Command failed with exit status:", os.WEXITSTATUS(return_value)
            )
